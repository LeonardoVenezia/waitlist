...
interface WidgetSettings {
  [key: string]: unknown;
  fields?: { collect_name: boolean; custom_fields: CustomField[]; field_order: string[] };
  layout?: { corner_radius: number; alignment: string; arrangement: string; full_width: boolean; font_size: number; border_width: number };
...
export function IntegrationClient({
  ...
  // ── Widget defaults ──
  const [wFields, setWFields] = useState(widget.fields ?? { collect_name: false, custom_fields: [], field_order: ["name", "email"] });
  const [wLayout, setWLayout] = useState(widget.layout ?? { corner_radius: 10, alignment: "center", arrangement: "stack", full_width: true, font_size: 15, border_width: 1 });
  ...
  const [wCustomHead, setWCustomHead] = useState(widget.custom_head ?? "");
  ...
  const [lbContent, setLbContent] = useState({
    ...
    users_limit: leaderboard.users_limit ?? 50,
  });
  ...
  const saveWidget = useCallback(async () => {
    ...
    router.refresh();
  }, [waitlistId, wFields, wLayout, wInput, wButton, wCustomHead, router]);
      ...
      {tab === "design" && (
        <div className="grid grid-cols-1 gap-5 lg:grid-cols-12">
          ...
          <div className="lg:col-span-7 space-y-4">
            ...
            <Section
              title="Fields"
              subtitle="What you collect from signups."
            ...
            >
              <div className="pt-4 space-y-2">
                {/* Name field */}
                <div className="flex items-center gap-3 px-2 py-1.5 rounded-lg hover:bg-muted/50">
                  <span className="text-sm text-muted-foreground flex-1">Name <span className="text-xs">text</span></span>
                  <button
                    type="button"
                    role="switch"
                    aria-checked={wFields.collect_name}
                    onClick={() => setWFields({ ...wFields, collect_name: !wFields.collect_name })}
                    className={`relative inline-flex h-5 w-9 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors ${wFields.collect_name ? "bg-primary" : "bg-muted-foreground/20"}`}
                  >
                    <span className={`inline-block size-4 rounded-full bg-white shadow-sm transition-transform ${wFields.collect_name ? "translate-x-4" : "translate-x-0"}`} />
                  </button>
                </div>
                {/* Email field (always on) */}
                <div className="flex items-center gap-3 px-2 py-1.5 rounded-lg">
                  <span className="text-sm flex-1">Email <span className="text-xs text-muted-foreground">email · required</span></span>
                </div>
                {/* Custom fields */}
                {wFields.custom_fields.map((cf) => (
                  <div key={cf.id} className="flex items-center gap-3 px-2 py-1.5 rounded-lg hover:bg-muted/50">
                    <span className="text-sm flex-1">{cf.label} <span className="text-xs text-muted-foreground">{cf.type}</span></span>
                    <button
                      onClick={() => setWFields({ ...wFields, custom_fields: wFields.custom_fields.filter((f) => f.id !== cf.id) })}
                ...
                <div className="relative">
                  <button
                    ...
                    onClick={() => {
                      ...
                      setWFields({ ...wFields, custom_fields: [...wFields.custom_fields, { id, type: "text", label: `Field ${wFields.custom_fields.length + 1}`, required: false }] });
                    }}
                    className="inline-flex items-center gap-1 px-1 py-1 text-xs font-medium text-muted-foreground hover:text-foreground rounded transition-colors"
                  >
            ...
            >
              <div className="pt-4 space-y-5 divide-y">
                ...
                {/* Arrangement */}
                <section className="py-5">
                  <h4 className="text-xs font-semibold tracking-wide uppercase">Arrangement</h4>
                  <div className="grid grid-cols-2 gap-2 mt-3">
                    {(["stack", "inline"] as const).map((a) => (
                      <label key={a} className="cursor-pointer">
                        <input type="radio" name="arrangement" value={a} checked={wLayout.arrangement === a} onChange={() => setWLayout({ ...wLayout, arrangement: a })} className="sr-only peer" />
                        <span className={`flex items-center gap-2 px-3 py-2.5 text-xs rounded-lg border transition ${wLayout.arrangement === a ? "text-primary bg-primary/5 border-primary" : "text-muted-foreground bg-muted/50 border-input hover:border-primary/30"}`}>
                          {a === "stack" ? "Stack" : "Inline"}
                        </span>
                      </label>
                    ))}
                  </div>
                ...
                {/* Width */}
                <section className="py-5">
                  <h4 className="text-xs font-semibold tracking-wide uppercase">Width</h4>
                  <div className="grid grid-cols-2 gap-2 mt-3">
                    <label className="cursor-pointer">
                      <input type="radio" name="fullWidth" checked={!wLayout.full_width} onChange={() => setWLayout({ ...wLayout, full_width: false })} className="sr-only peer" />
                      <span className={`flex items-center gap-2 px-3 py-2.5 text-xs rounded-lg border transition ${!wLayout.full_width ? "text-primary bg-primary/5 border-primary" : "text-muted-foreground bg-muted/50 border-input hover:border-primary/30"}`}>Auto</span>
                    </label>
                    <label className="cursor-pointer">
                      <input type="radio" name="fullWidth" checked={wLayout.full_width} onChange={() => setWLayout({ ...wLayout, full_width: true })} className="sr-only peer" />
                      <span className={`flex items-center gap-2 px-3 py-2.5 text-xs rounded-lg border transition ${wLayout.full_width ? "text-primary bg-primary/5 border-primary" : "text-muted-foreground bg-muted/50 border-input hover:border-primary/30"}`}>Full</span>
                    </label>
                  </div>
                </section>
            ...
            <Section
              title="Custom code"
              subtitle="Inject fonts, CSS, or scripts."
            ...
            >
              <div className="pt-4">
                <label className="block text-xs font-medium text-muted-foreground mb-1.5">Head code</label>
                <textarea
                  value={wCustomHead}
                  onChange={(e) => setWCustomHead(e.target.value)}
                  rows={3}
                  placeholder="Paste your code here"
                  className="block w-full px-3 py-2 text-xs font-mono border border-input rounded-lg focus:ring-1 focus:ring-primary/30 focus:border-primary bg-transparent"
                />
              </div>
            </Section>
          </div>
      ...
      {tab === "install" && (
        <div className="max-w-3xl">
          <div className="grid grid-cols-1 gap-3 md:grid-cols-2 mb-6">
            <div className="flex items-start w-full px-4 py-3 text-left rounded-xl bg-card border border-primary ring-1 ring-primary shadow-sm">
              <span className="inline-flex items-center justify-center flex-shrink-0 size-9 mr-3 rounded-lg bg-primary/10 text-primary">
            ...
            <div className="flex items-start w-full px-4 py-3 text-left rounded-xl bg-card border hover:border-foreground/20">
              <span className="inline-flex items-center justify-center flex-shrink-0 size-9 mr-3 rounded-lg bg-muted text-muted-foreground">
