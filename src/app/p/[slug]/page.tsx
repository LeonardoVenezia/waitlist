...
import { NeonTemplate } from "@/components/templates/neon-template";
import { CarbonTemplate } from "@/components/templates/carbon-template";
import { PastelTemplate } from "@/components/templates/pastel-template";
...
export default async function HostedPage(props: {
  ...
  const settings = (waitlist.settings as Record<string, unknown>) ?? {};
  const pageSections = (settings.page_sections as {
    template_id?: unknown;
  ...
  if (templateDefinition) {
    ...
    return (
      <div className="min-h-screen">
        ...
        {templateDefinition.id === "neon" && (
          <div className="min-h-screen bg-[#090A0F] flex items-center justify-center px-4 py-16">
            <NeonTemplate
              publicKey={waitlist.public_key}
              data={templateData as never}
              realCount={realCount}
            />
          </div>
        )}
        {templateDefinition.id === "carbon" && (
          <div
            className="min-h-screen bg-[#0B0C10] px-4 py-16"
            style={{
              backgroundImage:
